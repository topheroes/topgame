...
BubbleProblem.CollisionDetector = (function($){
	var CollisionDetector = {
		findIntersection : function(curBubble,board,angle){
			...
			var start = {
				left : pos.left + BubbleProblem.ui.BUBBLE_DIMS/2,
				top : pos.top + BubbleProblem.ui.BUBBLE_DIMS/2
			};
			...
			for(var i=0;i<rows.length;i++){
				...
				for(var j=0;j<row.length;j++){
					...
					if(bubble){
						...
						if(distEC<BubbleProblem.ui.BUBBLE_DIMS * .75){
							var dt = Math.sqrt(BubbleProblem.ui.BUBBLE_DIMS * BubbleProblem.
